...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	$sql2 = "SELECT TallerID FROM Taller WHERE Nombre = 'Cocina con la Valita'";

	if (mysqli_query($db, $sql)) {
		echo '<div class="notification is-primary"><button class="delete"></button>Te has registrado correctamente en el taller Cocina con la Valita, pronto recibirás un correo con la información de pago.</div>';
	} else {
	...
	if (mysqli_query($db, $sql2)) {
		...
		while($row = mysqli_fetch_array($resultado)) {
			...
			if (mysqli_query($db, $sql3)) {
				echo "Exito en registrar la tabla relacional";
			} else {
...
<body>
	<section class="section">
		<div class="container">
	      	<h1 class="title">
	        	Cocina con la Valita
	      	  </h1>
		  	<p>En este taller aprenderás a cocinar con la Valita</p>
		  	<br>
